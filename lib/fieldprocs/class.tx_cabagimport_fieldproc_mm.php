 ...
 */
class  tx_cabagimport_fieldproc_mm implements tx_cabagimport_ifieldproc  {
	...
	*/
	function main($stackPartConf=false, $object_handler) {
		...
		if(strlen($this->conf['split']) > 0) {
			...
			$referenceRecords = t3lib_div::trimExplode(',',$this->objectHandler->currentFieldValue, 1);
		}
		...
		foreach($finischedReferenceRecords as $key => $value) {
			...
			$relatedRecord = t3lib_BEfunc::getRecordRaw($this->conf['table'], $relatedRecordWhere);
