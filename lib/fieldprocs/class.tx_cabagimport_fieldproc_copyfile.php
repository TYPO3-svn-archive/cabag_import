 ...
 */
class  tx_cabagimport_fieldproc_copyfile implements tx_cabagimport_ifieldproc {
		...
		if(!empty($this->conf['sourcepath'])) {
			if(!empty($this->conf['destinationpath'])) {
				...
				if(!empty($this->conf['split'])){
					$sourcepathArray = t3lib_div::trimExplode($this->conf['split'],$this->conf['sourcepath'], 1);
					
					if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) t3lib_div::devLog('splited source', 'cabag_import', -1, $sourcepathArray);
				} else {
				...
				foreach($sourcepathArray as $key => $this->conf['sourcepath']){
					...
					if ($this->conf['allowedFiletypes']) {
						$allowedFiletypesArray = t3lib_div::trimExplode(',', $this->conf['allowedFiletypes'], 1);
						if (count($allowedFiletypesArray)) {
							...
							if (!$isAllowed && !empty($filename)) {
								if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) t3lib_div::devLog('not allowed filetype', 'cabag_import', 2, $filename);
								return FALSE;
							}
						}
					}
					...
					if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) t3lib_div::devLog('copy file', 'cabag_import', -1, array($this->conf['sourcepath'], $copyto));
					...
					if($object_handler->dryMode == false){
						...
						if($this->conf['dontCopyIfExists'] && file_exists($copyto)){
							...
							if(strstr($this->conf['sourcepath'], '://') !== false && $this->conf['useGetURL']) {
								$copyreturn = file_put_contents($copyto, t3lib_div::getURL($this->conf['sourcepath']));
							} else {
						...
						if($copyreturn){
							...
							if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) t3lib_div::devLog('file not found', 'cabag_import', 3, array($this->conf['sourcepath'], $copyto));
							throw new Exception($LANG->getLL('fieldProc.exception.fileNotFound').' - '.$this->conf['sourcepath']);
						}
					} else {
