 ...
 */
class  tx_cabagimport_fieldproc_mkdir implements tx_cabagimport_ifieldproc {
	...
	function main($stackPartConf=false, $object_handler) {
		...
		if(!is_dir($path)){
			if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) {
				t3lib_div::devLog('mkdir: folder '.$path.' does not exist, will try to create it', 'cabag_import', -1, $path);
			}
			
			if(empty($this->conf['deep'])) {
				if($object_handler->dryMode == false){
					if(t3lib_div::mkdir($path)) {
						if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) {
							t3lib_div::devLog('mkdir: folder '.$path.' created', 'cabag_import', -1, $path);
						}
					} else {
						if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) {
							t3lib_div::devLog('mkdir: could not create folder '.$path, 'cabag_import', -1, $path);
						}
					}
				}
			} else {
				if($object_handler->dryMode == false){
					$error = t3lib_div::mkdir_deep(PATH_site, $object_handler->currentFieldValue);
					if(strlen($error) > 0) {
						if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) {
							t3lib_div::devLog('mkdir: folder '.$path.' created', 'cabag_import', -1, $path);
						}
			...
			if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) {
				t3lib_div::devLog('mkdir: folder '.$path.' exists', 'cabag_import', -1, $path);
			}
		}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cabag_import/lib/class.tx_cabagimport_fieldproc_mkdir.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cabag_import/lib/class.tx_cabagimport_fieldproc_mkdir.php']);
}
?>
