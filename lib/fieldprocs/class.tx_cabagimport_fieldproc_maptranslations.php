 ...
 */
class  tx_cabagimport_fieldproc_maptranslations implements tx_cabagimport_ifieldproc {
	...
	*/
	function main($stackPartConf=false, $object_handler) {
		...
		$xmlArray = t3lib_div::xml2array($this->conf['flex']);		
		...
		foreach($this->conf['fields.'] as $field) {
			...
			if(is_array($xmlArray) && !empty($xmlArray['data']['sDEF']['lDEF'][$field]['vDEF'])){
				...
				$contentElementUids = t3lib_div::trimExplode(',', $oldUids, 1);
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cabag_import/lib/fieldproc/class.tx_cabagimport_fieldproc_maptranslations.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cabag_import/lib/fieldproc/class.tx_cabagimport_fieldproc_maptranslations.php']);
}
?>
