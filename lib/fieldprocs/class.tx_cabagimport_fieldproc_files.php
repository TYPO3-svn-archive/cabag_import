 ...
 */
class  tx_cabagimport_fieldproc_files implements tx_cabagimport_ifieldproc {
	...
	*/
	function main($stackPartConf, $object_handler) {
		...
		if (!empty($this->conf['useCustomCmd'])) {
			...
			// debug
			if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) t3lib_div::devLog('search file '.$cmd, 'cabag_import', -1, $this->filesInSource);
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cabag_import/lib/class.tx_cabagimport_fieldproc_files.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cabag_import/lib/class.tx_cabagimport_fieldproc_files.php']);
}
?>
