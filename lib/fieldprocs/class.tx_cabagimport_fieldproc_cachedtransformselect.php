 ...
 */
class  tx_cabagimport_fieldproc_cachedtransformselect implements tx_cabagimport_ifieldproc {
	...
	*/
	function main($stackPartConf=false, $object_handler) {
		...
		if(empty($this->conf['cacheid'])) {
			throw new Exception('no cachid set');
		}
		...
		if(empty($this->conf['transform'])) {
			throw new Exception('no transform value set');
		}
