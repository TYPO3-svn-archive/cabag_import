 ...
 */
class tx_cabagimport_storage_csv implements tx_cabagimport_istorage {
	/**
	...
	public function main($tx_cabagimport_handler, $conf = array()) {
		...
		$this->keys = t3lib_div::trimExplode(',', $this->handler->conf['handler.']['keyFields']);
	}
	...
	public function insertRow($row=false, $table=false) {
		...
		if ($this->nextRow === 0 && $this->conf['writeTitleRow']) {
			$titleRow = array();
	 ...
	 *
	 * @param boolean $throwException If set this function will throw an exception instead of return a boolean.
	 * @return boolean Whether or not this object can write at the moment.
	 */
	public function canWrite($throwException = false) {
		$canWrite = $this->file !== NULL || $this->isDryMode();

		if ($throwException && !$canWrite) {
