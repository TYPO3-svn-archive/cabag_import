 ...
 */
class  tx_cabagimport_source_file implements tx_cabagimport_isource {
	...
	*/
	function open($dryRun, $conf, $tx_cabagimport_handler) {
		...
		if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) t3lib_div::devLog('init file source', 'cabag_import', -1, $conf);
		...
		if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) t3lib_div::devLog('found file ' . $this->conf['filePath'], 'cabag_import', -1, $conf);
	...
	* @return	bool/string	true for success or an exception
	*/
	function checkArchivePath($archivePath) {
		global $LANG;
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cabag_import/lib/sources/class.tx_cabagimport_source_file.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cabag_import/lib/sources/class.tx_cabagimport_source_file.php']);
}
?>
