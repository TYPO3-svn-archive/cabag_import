 ...
 */
class  tx_cabagimport_source_mssql implements tx_cabagimport_isource {
	...
	*/
	function open($dryRun, $conf, $tx_cabagimport_handler) {
		...
		if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) t3lib_div::devLog('init sql source', 'cabag_import', -1, $conf);
		...
		if($this->resource = mssql_pconnect($this->conf['host'],$this->conf['user'],$this->conf['password'])) {
			if(!empty($this->conf['noDatabase']) || mssql_select_db($this->conf['database'],$this->resource)) {
				...
				if(!$this->queryResource) {
					throw new Exception($LANG->getLL('source.exception.noQueryResult'));
				}
			} else {
		...
		// debug
		if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) t3lib_div::devLog('connected to mssql host/db', 'cabag_import', -1, array('conf' => $this->conf));
	}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cabag_import/lib/sources/class.tx_cabagimport_source_mssql.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cabag_import/lib/sources/class.tx_cabagimport_source_mssql.php']);
}
?>
