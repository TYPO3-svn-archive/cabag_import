 ...
 */
class  tx_cabagimport_source_ldap implements tx_cabagimport_isource {
	...
	*/
	function open($dryRun, $conf, $tx_cabagimport_handler) {
		...
		if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) t3lib_div::devLog('init ldap source', 'cabag_import', -1, $conf);
		...
		if($count < 1) {
			...
			if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) t3lib_div::devLog('found serveral entries in ldap search', 'cabag_import', -1, array('count' => $count));
		}
		
		// debug
		if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) t3lib_div::devLog('connected to ldap server', 'cabag_import', -1, array('conf' => $this->conf));
	}
	...
	*/
	function getNextRow() {
		...
		// debug
		if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) t3lib_div::devLog('ldap return array', 'cabag_import', -1, $row);
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cabag_import/lib/sources/class.tx_cabagimport_source_mysql.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cabag_import/lib/sources/class.tx_cabagimport_source_mysql.php']);
}
?>
