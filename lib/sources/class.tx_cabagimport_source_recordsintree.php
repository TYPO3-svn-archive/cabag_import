 ...
 */
class  tx_cabagimport_source_recordsintree implements tx_cabagimport_isource {
	...
	*/
	function open($dryRun, $conf, $tx_cabagimport_handler) {
		...
		// debug
		if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) t3lib_div::devLog('selected records from tree', 'cabag_import', -1, array('conf' => $this->conf));
	}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cabag_import/lib/sources/class.tx_cabagimport_source_recordsintree.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cabag_import/lib/sources/class.tx_cabagimport_source_recordsintree.php']);
}
?>
