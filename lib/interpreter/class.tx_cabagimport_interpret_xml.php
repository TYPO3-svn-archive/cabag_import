 ...
 */
class  tx_cabagimport_interpret_xml implements tx_cabagimport_iinterpret {
	...
	*/
	function init($conf=false, $source=false) {
		...
		$this->xmlArray = t3lib_div::xml2tree($this->xmlRaw);
		$this->recordPath = t3lib_div::trimExplode(',', $this->conf['recordPath'], 1);
		...
		// debug
		if($TYPO3_CONF_VARS['SYS']['enable_DLOG']) t3lib_div::devLog('parsed xml', 'cabag_import', -1, array('conf' => $this->conf, 'raw' => $this->xmlRaw, 'xml' => $this->xmlArray));
	}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cabag_import/lib/class.tx_cabagimport_interpret_xml.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/cabag_import/lib/class.tx_cabagimport_interpret_xml.php']);
}
?>
