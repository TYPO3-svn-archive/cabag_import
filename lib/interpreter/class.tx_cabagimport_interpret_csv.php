 ...
 */
class  tx_cabagimport_interpret_csv implements tx_cabagimport_iinterpret {
	...
	*/
	function getNextRow() {
		...
		if(strlen(trim($this->conf['enclosure'])) > 0) {
			$arrayLine = fgetcsv($this->resource, 10000, $this->conf['delimiter'], trim($this->conf['enclosure']));
		} else {
			$arrayLine = fgetcsv($this->resource, 10000, $this->conf['delimiter']);
		}
